...
export default class Signaling {
    ...
    constructor(_numberToCall, _sipLogin, _sipPassword, _statusCallback) {
        if (_numberToCall && !this.numberToCall) {
            this.numberToCall = _numberToCall;
        }
        if (_sipLogin && !this.sipLogin) {
            this.sipLogin = _sipLogin;
        }
        if (_sipPassword && !this.sipPassword) {
    ...
    getToken(onSuccess, onError) {
        ...
        let data = JSON.stringify({
            "identifier": this.generateRamdomString(16),
            "login": this.sipLogin,
